...
class INFOGANCONV:
    ...
    def loss(self,x_in,y_in,z_in):
        ...
        c_labels_fake = self.c(generated,training = True,name = 'c')

        d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_real,labels = tf.ones_like(d_logits_real)))
        d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_fake,labels = tf.zeros_like(d_logits_fake)))
        d_loss = d_loss_fake+d_loss_real
        g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_fake,labels = tf.ones_like(d_logits_fake)))
        q_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=c_labels_fake,labels = y_in))
    ...
    def train(self,losses,learning_rate = 0.0002):
        ...
        q_opt_op = tf.train.AdamOptimizer(learning_rate).minimize(losses["q_loss"],var_list = self.g.variables+self.c.variables)

        with tf.control_dependencies([d_opt_op,g_opt_op,q_opt_op]):
